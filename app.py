...
- Target database: ipl_csk_players.
- Table: ipl_csk_players.
...
A: SELECT COUNT(*) FROM ipl_csk_players
Q: Show all Allrounder
A: SELECT * FROM ipl_csk_players WHERE players_role = 'Allrounder'

Q: Who earns more than 600000?
A: SELECT * FROM ipl_csk_players WHERE players_salary > 600000

Q: Who earns the highest salary?
A: SELECT * FROM ipl_csk_players ORDER BY players_salary DESC LIMIT 1
...
st.title("Gemini SQL Assistant (CSK Players DB)")
...
if st.button("Run"):
    if question.strip() == "":
        ...
        # Run SQL query
        result, columns = read_sql_query(sql_query, "ipl_csk_players.db")
